...
const Videos = () => {
  ...
  return (
    ...
    >
      <div className="absolute left-0 top-0 h-[440px] w-[440px] rounded-full bg-[var(--accent)]/10 blur-[120px]" />
            ...
            {tabs.map((tab, index) => (
              <button
                ...
                className={`group w-full rounded-3xl border p-5 text-left transition-all duration-300 ${
                  activeTab === index
                    ? "border-[var(--accent)]/35 bg-[var(--accent)]/10 shadow-[0_18px_60px_rgba(168,117,34,0.12)]"
                    : "border-zinc-100 bg-white shadow-sm hover:border-[var(--accent)]/30"
                }`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition ${
                      activeTab === index
                        ...
                        : "bg-zinc-100 text-zinc-800 group-hover:bg-[var(--accent)] group-hover:text-black"
                    }`}
                  >
                  ...
                  <ArrowRight className="ml-auto mt-1 h-5 w-5 shrink-0 text-zinc-400 transition group-hover:text-[var(--accent)]" />
                </div>
              </button>
            ))}
          </div>
